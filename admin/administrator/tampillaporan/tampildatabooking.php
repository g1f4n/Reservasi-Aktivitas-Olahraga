...
$mulai = $baris * ($hal - 1);
 ...
 <form action="" method="POST">
	<h4>Laporan Data Pemesan</h4>
	<input type="text" name="inputan_pencarian" placeholder="Masukan Kode Pemesan" style="width: 200px; padding: 5px; margin-left: 10px;">
...
<table class="tabel2">
	...
	if ($inputan_pencarian != "") {
		$sql = mysql_query("SELECT * FROM transaksi JOIN (member JOIN jenisolahraga JOIN instruktur) ON transaksi.kodemember = member.kodemember AND transaksi.kodeolahraga = jenisolahraga.kodeolahraga AND jenisolahraga.kodeinstruktur = instruktur.kodeinstruktur WHERE kodebooking LIKE '%$inputan_pencarian%'") or die(mysql_error());
	} else {
		$sql = mysql_query("SELECT * FROM transaksi JOIN (member JOIN jenisolahraga JOIN instruktur) ON transaksi.kodemember = member.kodemember AND transaksi.kodeolahraga = jenisolahraga.kodeolahraga AND jenisolahraga.kodeinstruktur = instruktur.kodeinstruktur WHERE status = 'Proses' ORDER BY kodebooking ASC LIMIT $mulai, $baris") or die(mysql_error());
	}
		...
		while ($data = mysql_fetch_array($sql)) {
			 if($data['status'] == 'Proses') { ?>
				<tr>
					...
					<td class="tampilan">
						<a href="../kasir/tampillaporan-solo/laporanbooking.php?kodebooking=<?php echo $data['kodebooking']; ?>"><i class="fa fa-print" title="Cetak"></i></a>
						<?php 
			}
					 ?>
				</td>
